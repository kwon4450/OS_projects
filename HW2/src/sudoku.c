...
void *check_rows(void *arg)
{
    for(int i=0; i<9; i++){
        int check = 0;
        for(int j=0; j<9; j++){
            check |= (1<<(sudoku[i][j]-1));
        }
        if(check==0b111111111) valid[0][i] = 1;
        else valid[0][i] = 0;
    }
    pthread_exit(NULL);
}
...
void check_sudoku(void)
{
     ...
     * 
     * 넘겨줄 정수 만들기 3*i+j 
     * ex) (0 = 0,0) (1 = 0,3) (2 = 0,6) (3 = 3, 0) (4 = 3, 3) ...
     * 3로 나눈 몫*3은 row 나머지*3은 col
     */
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 3; j++) {
            gid[3*i + j] = 3*i + j;
            if (pthread_create(&subgridtid[3*i + j], NULL, check_subgrid, &gid[3*i + j]) != 0) {
